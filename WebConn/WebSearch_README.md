...
3. **Answer Generation:** The extracted text context is fed back into the local Qwen model alongside the original question. Qwen then synthesizes a final, grounded answer based on the real-time web data.
...
* **`websrch.py`**: The core service script. It loads the local Qwen model, orchestrates the 3-step pipeline, communicates with the MCP server, and implements the BeautifulSoup fallback logic.
...
Run the parent script with your question. You can optionally specify the path to your local Qwen model.
