...
def main():
    ...
    output_parquet = sys.argv[2] if len(sys.argv) > 2 else os.path.join("Datasets", "nvidia_compute_eval_new.parquet")
    output_txt = sys.argv[3] if len(sys.argv) > 3 else os.path.join("Datasets", "nvidia_compute_eval_new.txt")
    ...
    with open(output_txt, "w", encoding="utf-8") as f_out:
        for line_idx, row in enumerate(records, start=1):
            ...
            system_prompt = "You are an expert GPU C++ developer. Write optimized code to solve the user's problem.\n\n"
            problem_content = f"{system_prompt}### WORKSPACE CONTEXT ###\n{workspace}\n\n### ENVIRONMENT ###\n{environment}\n\n### INSTRUCTION ###\n{prompt}\n"
    ...
    print(f"Successfully formatted {len(records)} records into 2-column format to {output_parquet}.")
